...
class Client(models.Model):
    ...
    home_phone = models.CharField(max_length=20, name="homePhone", blank=True, null=True, unique=True,
                                  error_messages={'unique': "Клиент с таким домашним номером уже существует"})
    mobile_phone = models.CharField(max_length=20, name="mobilePhone", blank=True, null=True, unique=True,
                                    error_messages={'unique': "Клиент с таким мобильным номером уже существует"})
    email = models.CharField(max_length=50, name="email", blank=True, null=True, unique=True,
                             error_messages={'unique': "Клиент с такой электронной почтой уже существует"})
    ...
    passport_number_id = models.CharField(max_length=14, name="passportNumberId", blank=False, null=False, unique=True,
                                          error_messages={'unique': "Клиент с таким идентификационым номером уже существует"})
